...
yalnizMasaustu("ürün detay galerisi masaüstü düzeninde");
...
const PAGES: PageDef[] = [
  ...
  {
    name: "messages",
    path: "/pages/dashboard/messages.html",
    xdata: "messagesComponent",
    gated: true,
  },
  {
    name: "orders",
    path: "/pages/dashboard/orders.html",
    xdata: "ordersListComponent",
    gated: true,
  },
  {
    name: "payment",
    path: "/pages/dashboard/payment.html",
    xdata: "paymentManagement",
    gated: true,
  },
  {
    name: "settings",
    path: "/pages/dashboard/settings.html",
    xdata: "settingsLayout",
    gated: true,
  },
  {
    name: "addresses",
    path: "/pages/dashboard/addresses.html",
    xdata: "addressesManager",
    gated: true,
  },
  {
    name: "buyer-dashboard",
    path: "/pages/dashboard/buyer-dashboard.html",
    xdata: "buyerUserInfo",
    gated: true,
  },
  {
    name: "application-pending",
    path: "/pages/seller/application-pending.html",
    xdata: "applicationPendingPage",
    gated: true,
  },
  {
    name: "seller-storefront",
    path: "/pages/seller/seller-storefront.html",
    xdata: "sellerStorefront",
    gated: false,
  },
  {
    name: "seller-dashboard",
    path: "/pages/seller/dashboard.html",
    xdata: "sellerDashboard",
    gated: false,
  },
  {
    name: "seller-shop",
    path: "/pages/seller/seller-shop.html",
    xdata: "sellerShop",
    gated: false,
  },
  ...
  { name: "register", path: "/pages/auth/register.html", xdata: "registerPage", gated: false },
  {
    ...
    xdata: "forgotPasswordPage",
    gated: false,
  },
  {
    name: "reset-password",
    path: "/pages/auth/reset-password.html",
    xdata: "resetPasswordPage",
    gated: false,
  },
  {
    name: "accept-invite",
    path: "/pages/auth/accept-invite.html",
    xdata: "acceptInvitePage",
    gated: false,
  },
  ...
  {
    name: "help-ticket-new",
    path: "/pages/help/help-ticket-new.html",
    xdata: "ticketForm",
    gated: true,
  },
  {
    name: "help-tickets",
    path: "/pages/help/help-tickets.html",
    xdata: "ticketsList",
    gated: true,
  },
  ...
  { name: "top-deals", path: "/pages/top-deals.html", xdata: "loginModal", gated: false },
  {
    ...
    path: "/pages/tailored-selections.html",
    xdata: "loginModal",
    gated: false,
  },
...
for (const p of PAGES) {
  test(`smoke: ${p.name} (x-data="${p.xdata}")`, async ({ page }) => {
    ...
    const capture = (t: string) => {
      const line = t.split("\n")[0];
      if (isAlpineErr.test(line)) {
        alpineErrors.push(line.slice(0, 80));
        if (process.env.SMOKE_DUMP)
          console.error(`\n[DUMP ${p.name}] url=${page.url()}\nFULL: ${t.slice(0, 400)}\n`);
      }
    };
    page.on("console", (msg) => {
      if (msg.type() === "error" || msg.type() === "warning") capture(msg.text());
    });
    page.on("pageerror", (err) => capture(String(err)));

    if (p.seedCart) {
      await page.addInitScript(
        (c) => localStorage.setItem("tradehub_cart", JSON.stringify(c)),
        SEED_CART
      );
    }
    ...
    if (/pages\/auth\/login\.html|\/giris/.test(finalUrl)) {
      throw new Error(
        `${p.name}: login'e redirect oldu (oturum geçersiz?) — doğrulanamadı. URL=${finalUrl}`
      );
    }
    ...
    expect(
      hydrated.hydrated,
      `${p.name}: x-data="${p.xdata}" Alpine tarafından hydrate EDİLMEDİ (modül register olmadı?)`
    ).toBeTruthy();
