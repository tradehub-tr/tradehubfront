...
function themeBootstrapPlugin(): Plugin {
  ...
  const inlineScript = `<script>(function(){try{var k='tradehub-theme-remote';var o=JSON.parse(localStorage.getItem(k)||'{}');var r=document.documentElement;function apply(m){for(var n in m){if(typeof n==='string'&&n.indexOf('--')===0&&typeof m[n]==='string'){r.style.setProperty(n,m[n]);}}}apply(o);if(typeof fetch==='function'){fetch('/api/method/tradehub_core.api.theme.get_public_theme',{credentials:'same-origin'}).then(function(x){return x.ok?x.json():null;}).then(function(d){if(!d||!d.message)return;var ov=d.message.overrides||{};var clean={};for(var q in ov){if(typeof q==='string'&&q.indexOf('--')===0&&typeof ov[q]==='string')clean[q]=ov[q];}try{localStorage.setItem(k,JSON.stringify(clean));}catch(e){}for(var s in o){if(!(s in clean)&&s.indexOf('--')===0){r.style.removeProperty(s);}}apply(clean);}).catch(function(){});}}catch(e){}})();</script>`;
  return {
...
function fontHeadPlugin(): Plugin {
  ...
  const fontCss =
    "@font-face{font-family:'Inter';font-style:normal;font-weight:300 800;font-display:swap;src:url(/fonts/inter-cyrillic-ext.woff2) format('woff2');unicode-range:U+0460-052F, U+1C80-1C8A, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F}@font-face{font-family:'Inter';font-style:normal;font-weight:300 800;font-display:swap;src:url(/fonts/inter-cyrillic.woff2) format('woff2');unicode-range:U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116}@font-face{font-family:'Inter';font-style:normal;font-weight:300 800;font-display:swap;src:url(/fonts/inter-latin-ext.woff2) format('woff2');unicode-range:U+0100-02BA, U+02BD-02C5, U+02C7-02CC, U+02CE-02D7, U+02DD-02FF, U+0304, U+0308, U+0329, U+1D00-1DBF, U+1E00-1E9F, U+1EF2-1EFF, U+2020, U+20A0-20AB, U+20AD-20C0, U+2113, U+2C60-2C7F, U+A720-A7FF}@font-face{font-family:'Inter';font-style:normal;font-weight:300 800;font-display:swap;src:url(/fonts/inter-latin.woff2) format('woff2');unicode-range:U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+0304, U+0308, U+0329, U+2000-206F, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD}";
  // Birincil arayüz fontu (istoc-sans): 3 statik ağırlık — 400 gövde, 600 başlık,
  // 700 fiyat. Latin + Latin-Ext (TR tam) + Kiril (RU tam) tek dosyada, bu yüzden
  // unicode-range YOK — eksik glifte tarayıcı kendiliğinden Inter'e düşer.
  // Inter fallback olarak korunuyor: Arapça ikisinde de yok, sistem fontuna düşer.
  const uiFontCss =
    "@font-face{font-family:'istoc-sans';font-style:normal;font-weight:400;font-display:swap;src:url(/fonts/istoc-sans-400.woff2) format('woff2')}" +
    "@font-face{font-family:'istoc-sans';font-style:normal;font-weight:500;font-display:swap;src:url(/fonts/istoc-sans-600.woff2) format('woff2')}" +
    "@font-face{font-family:'istoc-sans';font-style:normal;font-weight:600;font-display:swap;src:url(/fonts/istoc-sans-600.woff2) format('woff2')}" +
    "@font-face{font-family:'istoc-sans';font-style:normal;font-weight:700;font-display:swap;src:url(/fonts/istoc-sans-700.woff2) format('woff2')}";
  const fontLinks = [
    '<link rel="preload" href="/fonts/istoc-sans-400.woff2" as="font" type="font/woff2" crossorigin />',
    '<link rel="preload" href="/fonts/istoc-sans-700.woff2" as="font" type="font/woff2" crossorigin />',
    `<style>${uiFontCss}${fontCss}</style>`,
  ].join("\n    ");
  return {
    ...
    transformIndexHtml: {
      ...
      async handler(html, ctx) {
        ...
        if (!/href="\/images\/istoc-favicon-96\.png"/i.test(out)) {
          tags.push(
            '<link rel="icon" type="image/png" sizes="96x96" href="/images/istoc-favicon-96.png" />'
          );
        }
        if (!/href="\/images\/istoc-favicon-48\.png"/i.test(out)) {
          tags.push(
            '<link rel="icon" type="image/png" sizes="48x48" href="/images/istoc-favicon-48.png" />'
          );
        }
        if (NOINDEX_FILES.has(rel)) {
          ...
          out = out
            ...
            .replace(
              /[ \t]*<meta\s+property="og:(?:title|description|url|image(?::width|:height)?)"[^>]*>\s*\n?/gi,
              ""
            );
          ...
          if (meta.prettyPath) {
            const canonical =
              meta.prettyPath === "/"
                ? `${CANONICAL_ORIGIN}/`
                : `${CANONICAL_ORIGIN}${meta.prettyPath}`;
  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        ...
        short_name: "istoc",
        description:
          "Doğrulanmış satıcılar, güvenli ödeme ve şeffaf komisyonla küresel B2B toptan ticaret.",
        lang: "tr",
        dir: "ltr",
        ...
        shortcuts: [
          ...
          },
          {
            ...
            url: "/pages/dashboard/orders.html",
            icons: [{ src: "/icons/icon-192.png", sizes: "192x192" }],
          },
        ],
      },
    ...
    rollupOptions: {
      ...
      output: {
        manualChunks(id) {
          ...
          if (id.includes("node_modules/@tanstack") || id.includes("node_modules/idb-keyval"))
          ...
          if (
            id.includes("src/alpine/") &&
            !id.includes("src/alpine/checkout.ts") &&
            !id.includes("src/alpine/kyb.ts") &&
            !id.includes("src/alpine/messages.ts") &&
            !id.includes("src/alpine/remittance.ts") &&
            !id.includes("src/alpine/orderItemsDrawer.ts") &&
            !id.includes("src/alpine/sellerShop.ts") &&
            ...
            !id.includes("src/alpine/auth.ts") &&
            !id.includes("src/alpine/products-filter.ts") &&
            !id.includes("src/alpine/cart.ts") &&
            !id.includes("src/alpine/help.ts") &&
            !id.includes("src/alpine/product.ts") &&
            !id.includes("src/alpine/loginModal.ts") &&
            !id.includes("src/alpine/orderProtectionModal.ts") &&
            !id.includes("src/alpine/sidebar.ts")
          )
            return "alpine";
        },
      },
