...
function messageToView(m: ChatwootMessage, conversationId: string): Message {
	...
	return {
		...
		body,
		time: toHHMM(date),
		dateLabel: date ? date.toLocaleDateString("tr-TR", { day: "numeric", month: "short" }) : undefined,
		read: true,
...
function absolutizeAttachmentUrl(url: string): string {
	if (!url) return url;
	return url.replace(/^https?:\/\/0\.0\.0\.0(:\d+)?\//, (_m, port) => `http://localhost${port || ""}/`);
}
...
export async function sendTextMessage(
	...
	const m = (raw && "message" in (raw as Record<string, unknown>)
