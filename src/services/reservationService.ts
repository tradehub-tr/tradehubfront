...
export async function listSellerSlots(sellerId: string): Promise<AvailableSlot[]> {
  const r = await gget<AvailableSlot[]>("tradehub_core.api.reservation.list_seller_slots", {
    seller_id: sellerId,
  });
...
export async function listMyReservations(includePast = false): Promise<Reservation[]> {
  const r = await gget<Reservation[]>("tradehub_core.api.reservation.list_my_reservations", {
