...
Alpine.data("messagesComponent", () => ({
  ...
  async _loadConversations() {
    ...
    try {
      ...
      const threads = await callMethod<RawThread[]>("tradehub_core.api.chat.list_my_threads", {
        perspective: "seller",
      });
  ...
  async _loadActiveMessages() {
    ...
    try {
      const raw = await callMethod<unknown>("tradehub_core.api.chat.list_messages", {
        conversation_id: id,
        perspective: "seller",
      });
  ...
  async sendMessage() {
    ...
    try {
      ...
      this.selectedConversation.messages = [...this.selectedConversation.messages, uiMsg];
