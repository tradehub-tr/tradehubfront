...
Alpine.data("sellPage", () => ({
  ...
  get businessTypes() {
    return t("sellerForm.businessTypes", { returnObjects: true }) as unknown as string[];
  },
  get categoryOptions() {
    return t("sellerForm.categoryOptions", { returnObjects: true }) as unknown as string[];
  },
  ...
  validateStep(step: number): boolean {
    ...
    if (step === 1) {
      if (!this.formData.companyName.trim())
        this.formErrors.companyName = t("sellerForm.companyNameRequired");
      if (!this.formData.businessType)
        this.formErrors.businessType = t("sellerForm.businessTypeRequired");
      if (!this.formData.taxNumber.trim())
        this.formErrors.taxNumber = t("sellerForm.taxNumberRequired");
    } else if (step === 2) {
      if (!this.formData.contactName.trim())
        this.formErrors.contactName = t("sellerForm.contactNameRequired");
      if (!this.formData.email.trim()) this.formErrors.email = t("sellerForm.emailRequired");
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.formData.email))
        this.formErrors.email = t("sellerForm.emailInvalid");
      if (!this.formData.phone.trim()) this.formErrors.phone = t("sellerForm.phoneRequired");
    } else if (step === 3) {
      if (this.formData.mainCategories.length === 0)
        this.formErrors.mainCategories = t("sellerForm.categoryMinOne");
    } else if (step === 4) {
      if (!this.formData.termsAccepted)
        this.formErrors.termsAccepted = t("sellerForm.termsRequired");
    }
  ...
  async submitForm() {
    ...
    try {
      ...
      await callMethod(
        "tradehub_core.api.seller.become_seller",
        {
  ...
  async init() {
    ...
    try {
      const [sellerRes, catRes] = await Promise.all([
        ...
        fetch(
          `${apiBase}/method/tradehub_core.api.seller.get_seller_categories?seller_code=${code}`,
          { credentials: "omit" }
        ).then((r) => r.json()),
      ]);
...
Alpine.data("sellerDashboard", () => ({
  ...
  saving: { account: false, company: false, contact: false, gallery: false } as Record<
    string,
    boolean
  >,
  ...
  async init() {
    try {
      ...
      const data = (await res.json()) as {
        message: { is_guest: boolean; is_seller?: boolean; seller?: Record<string, unknown> };
      };
  ...
  async saveSection(section: "account" | "company" | "contact") {
    ...
    try {
      ...
      // profile'ı güncelle
      const pr = (await this._post("tradehub_core.api.seller.get_my_profile")) as Record<
        string,
        unknown
      >;
      this.profile = pr;
  ...
  // ─── Ürünler ───────────────────────────────────────────────
  async loadProducts() {
    const data = (await this._post("tradehub_core.api.seller.get_products")) as Record<
      string,
      unknown
    >[];
    this.products = data;
  ...
  async saveProduct() {
    ...
    try {
      if (this.productModal.editId) {
        await this._post("tradehub_core.api.seller.update_product", {
          product_id: this.productModal.editId,
          data: this.productModal.data,
        });
      } else {
        await this._post("tradehub_core.api.seller.create_product", {
          data: this.productModal.data,
        });
      }
  ...
  async loadCategories() {
    const data = (await this._post("tradehub_core.api.seller.get_categories")) as Record<
      string,
      unknown
    >[];
  ...
  async saveCategory() {
    ...
    try {
      if (this.categoryModal.editId) {
        await this._post("tradehub_core.api.seller.update_category", {
          category_id: this.categoryModal.editId,
          data: this.categoryModal.data,
        });
      } else {
        await this._post("tradehub_core.api.seller.create_category", {
          data: this.categoryModal.data,
        });
      }
  ...
  async loadGallery() {
    try {
      const data = (await this._post("tradehub_core.api.seller.get_gallery")) as {
        name: string;
        image: string;
        caption: string;
      }[];
