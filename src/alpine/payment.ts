...
import {
  ...
  exportTransactions,
  fetchBankInteractions,
  fetchBankInteractionSummary,
  verifySupplierAccount,
  fetchWireTransfers,
...
Alpine.data('bankAccounts', () => ({
  interactions: [] as any[],
  loading: true,
  activeTab: 'all' as string,
  summary: { total_wire_amount: 0, pending_match_amount: 0 },
  filters: {
    search: '',
    dateFrom: '',
    dateTo: '',
  ...
  async init() {
    await Promise.all([this.loadInteractions(), this.loadSummary()])
  },
  ...
  async loadInteractions() {
    ...
    try {
      const params: Record<string, any> = {}
      if (this.activeTab !== 'all') params.match_status = this.activeTab
      if (this.filters.search) params.search = this.filters.search
      if (this.filters.dateFrom) params.date_from = this.filters.dateFrom
      if (this.filters.dateTo) params.date_to = this.filters.dateTo

      const res = await fetchBankInteractions(params)
      this.interactions = res.interactions || []
    } catch (e) {
      console.error('Bank interactions load error:', e)
    } finally {
      this.loading = false
    }
  },
  ...
  async loadSummary() {
    try {
      const res = await fetchBankInteractionSummary()
      this.summary = {
        total_wire_amount: res.total_wire_amount || 0,
        pending_match_amount: res.pending_match_amount || 0,
      }
    } catch (e) {
  ...
  async verifyAccount() {
    if (!this.verifyIban.trim()) return
    this.verifying = true
    this.verifyResult = null
    try {
      this.verifyResult = await verifySupplierAccount(this.verifyIban.trim())
    } catch (e) {
      this.verifyResult = { verified: false, message: 'Doğrulama sırasında bir hata oluştu.' }
    } finally {
...
Alpine.data('wireTracking', () => ({
  ...
  async loadTransfers() {
    ...
    try {
      ...
      const res = await fetchWireTransfers(params)
      this.transfers = res.transfers || []
    } catch (e) {
      console.error('Wire transfers load error:', e)
    } finally {
      this.loading = false
    }
  },
