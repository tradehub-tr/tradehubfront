...
Alpine.data("filterSidebar", (initialCollapsed: Record<string, boolean> = {}) => ({
  ...
  init() {
    (this.$el as HTMLElement)
      .querySelectorAll<HTMLInputElement>('[data-filter-section][type="radio"]')
      .forEach((radio) => {
  ...
  handleSearchInput(event: Event) {
    ...
    const wrapper =
      input.closest("[data-filter-section-wrapper]") || input.parentElement?.parentElement;
    if (!wrapper) return;
  ...
  clearAllFilters() {
    (this.$el as HTMLElement)
      .querySelectorAll<HTMLInputElement>("[data-filter-section]")
      .forEach((input) => {
    ...
    (this.$el as HTMLElement)
      .querySelectorAll<HTMLLabelElement>('label[for^="filter-"]')
      .forEach((label) => {
        label.style.display = "";
      });
...
Alpine.data("filterChips", () => ({
  removeChipFilter(section: string, value: string) {
    ...
    if (section === "price") {
      document
        .querySelectorAll<HTMLInputElement>('[data-filter-section="price"][data-filter-type]')
        .forEach((i) => {
          i.value = "";
        });
    ...
    if (section === "min-order") {
      document
        .querySelectorAll<HTMLInputElement>('[data-filter-section="min-order"][data-filter-type]')
        .forEach((i) => {
          i.value = "";
        });
      document.dispatchEvent(new CustomEvent("filter-change"));
