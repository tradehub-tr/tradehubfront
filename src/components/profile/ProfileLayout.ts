...
function renderInfoField(field: InfoField): string {
  ...
  return `
    <div class="text-[13px] py-1.5" style="color:var(--color-text-tertiary)">${field.label}</div>
    <div class="text-[13px] py-1.5 flex items-center gap-1.5" style="color:${isEmpty ? "var(--color-text-tertiary)" : "var(--color-text-primary)"}">${valueHtml}</div>
  `;
}
  ...
  return `
    <div class="bg-white rounded-lg py-6 px-8 max-md:p-5 max-sm:p-4">
      <div class="flex items-center justify-between">
        ...
        <a href="#" class="inline-flex items-center gap-1 text-[13px] text-blue-600 no-underline font-medium transition-colors [@media(hover:hover)_and_(pointer:fine)]:hover:text-blue-700">
          ${ICONS.edit}
  ...
  return `
    <div class="flex items-start justify-between gap-6 bg-white rounded-b-lg py-6 px-8 -mt-px max-md:flex-col max-md:p-5 max-md:gap-4">
      ...
      <div class="flex items-center gap-3 flex-shrink-0 max-md:w-full">
        <a href="#" class="inline-flex items-center gap-1.5 py-2 px-4 rounded-md text-[13px] font-medium no-underline transition-colors duration-150 motion-reduce:transition-none whitespace-nowrap bg-surface-raised [@media(hover:hover)_and_(pointer:fine)]:hover:bg-gray-200 max-md:flex-1 max-md:justify-center" style="color:var(--color-text-primary)">
          ${ICONS.eye}
...
function renderFormField(field: FormField): string {
  ...
  if (field.isVerified) {
    valueHtml = `
      ...
      <span class="inline-flex items-center gap-[3px] text-xs italic" style="color:var(--color-primary-500)">${ICONS.verified} ${t("profile.emailVerified")}</span>
    `;
  } else {
    valueHtml = `<span style="color:var(--color-text-primary)">${field.value || ""}</span>`;
  }
...
function renderAccountEditForm(): string {
  return `
    <div class="bg-white rounded-lg p-8 max-md:p-5">
      <div class="flex items-center justify-between">
        <h2 class="text-lg font-semibold m-0" style="color:var(--color-text-primary)">${t("profile.editAccountInfo")}</h2>
