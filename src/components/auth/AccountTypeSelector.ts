...
export function AccountTypeSelector(
  ...
  return `
    <div id="account-type-selector" class="auth-account-type-grid flex flex-col gap-3">
      ...
      <button
        type="button"
        class="auth-account-type-card flex items-center gap-3.5 w-full py-4 px-[18px] rounded-md text-start relative cursor-pointer transition-[border-color,box-shadow,background-color] duration-150 ease-out motion-reduce:transition-none border-[1.5px] border-[var(--color-border-default,#e5e5e5)] bg-[var(--color-surface,#fff)] before:content-[''] before:shrink-0 before:w-5 before:h-5 before:border-2 before:border-[var(--color-border-medium,#d1d5db)] before:rounded-full before:transition-[border-color,background-color] before:duration-150 before:ease-[cubic-bezier(0.23,1,0.32,1)] before:motion-reduce:transition-none hover:border-[var(--color-border-medium,#d1d5db)] hover:bg-[var(--color-surface-muted,#fafafa)] focus-visible:outline-2 focus-visible:outline-[var(--color-cta-primary,#cc6b00)] focus-visible:outline-offset-2 [&.selected]:border-[#1f2937] [&.selected]:shadow-[0_0_0_1px_#1f2937] [&.selected]:before:border-[#1f2937] [&.selected]:before:bg-[#1f2937] [&.selected]:hover:bg-[var(--color-surface,#fff)]${defaultType === "buyer" ? " selected" : ""}${buyerLocked ? lockedClasses : ""}"
        ...
        role="radio"
      >
        <div class="auth-account-type-icon shrink-0 order-1 w-9 h-9 transition-colors text-[var(--color-text-placeholder,#999)] [&_svg]:w-full [&_svg]:h-full [.auth-account-type-card.selected_&]:text-[#1f2937]">
        ...
        <div class="auth-account-type-content flex flex-col gap-0.5 flex-1 min-w-0">
          <span class="auth-account-type-label text-[15px] font-semibold leading-tight text-[var(--color-text-heading,#111827)]">${t("auth.buyer")}</span>
          <span class="auth-account-type-desc text-[13px] leading-snug text-[var(--color-text-muted,#666)]">${t("auth.buyerDesc")}</span>
        </div>
      </button>
      ...
      <button
        type="button"
        class="auth-account-type-card flex items-center gap-3.5 w-full py-4 px-[18px] rounded-md text-start relative cursor-pointer transition-[border-color,box-shadow,background-color] duration-150 ease-out motion-reduce:transition-none border-[1.5px] border-[var(--color-border-default,#e5e5e5)] bg-[var(--color-surface,#fff)] before:content-[''] before:shrink-0 before:w-5 before:h-5 before:border-2 before:border-[var(--color-border-medium,#d1d5db)] before:rounded-full before:transition-[border-color,background-color] before:duration-150 before:ease-[cubic-bezier(0.23,1,0.32,1)] before:motion-reduce:transition-none hover:border-[var(--color-border-medium,#d1d5db)] hover:bg-[var(--color-surface-muted,#fafafa)] focus-visible:outline-2 focus-visible:outline-[var(--color-cta-primary,#cc6b00)] focus-visible:outline-offset-2 [&.selected]:border-[#1f2937] [&.selected]:shadow-[0_0_0_1px_#1f2937] [&.selected]:before:border-[#1f2937] [&.selected]:before:bg-[#1f2937] [&.selected]:hover:bg-[var(--color-surface,#fff)]${defaultType === "supplier" ? " selected" : ""}${supplierLocked ? lockedClasses : ""}"
      ...
      >
        ...
        <div class="auth-account-type-content flex flex-col gap-0.5 flex-1 min-w-0">
          <span class="auth-account-type-label text-[15px] font-semibold leading-tight text-[var(--color-text-heading,#111827)]">${t("auth.supplier")}</span>
          <span class="auth-account-type-desc text-[13px] leading-snug text-[var(--color-text-muted,#666)]">${t("auth.supplierDesc")}</span>
        </div>
      </button>
    </div>
  `;
