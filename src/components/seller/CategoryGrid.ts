        ...
        <div class="category-grid__row-top grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-4">
          ${topRow
            .map(
              (card) => `
            <a href="${card.link || "#"}"
               class="category-grid__card relative rounded-(--radius-lg) overflow-hidden min-h-[180px] group cursor-pointer hover:shadow-md [@media(hover:hover)and(pointer:fine)]:hover:scale-[1.03] dark:hover:shadow-lg transition-[transform,box-shadow] duration-300 motion-reduce:transition-none motion-reduce:transform-none focus-visible:ring-2 focus-visible:ring-(--store-accent)"
          ...
          <div class="category-grid__row-bottom grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ${bottomRow
              .map(
                (card) => `
              <a href="${card.link || "#"}"
                 class="category-grid__card relative rounded-(--radius-lg) overflow-hidden min-h-[160px] group cursor-pointer hover:shadow-md [@media(hover:hover)and(pointer:fine)]:hover:scale-[1.03] dark:hover:shadow-lg transition-[transform,box-shadow] duration-300 motion-reduce:transition-none motion-reduce:transform-none focus-visible:ring-2 focus-visible:ring-(--store-accent)"
                 style="background-color: ${card.bgColor}">
