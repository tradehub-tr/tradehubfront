              ...
              <div class="mb-2.5 sm:mb-3 border border-[#e5e5e5] rounded-md overflow-hidden">
                ...
                <template x-for="product in order.products" :key="product.id">
                  <div class="flex items-start gap-2 sm:gap-3 px-2.5 sm:px-3 py-2 border-b border-[#f3f4f6] last:border-b-0">
                    <img :src="product.image" :data-product-image-listing="product.id" :data-product-image-source="product.image" data-product-image-sizes="(min-width: 640px) 40px, 36px" width="40" height="40" decoding="async" class="w-9 h-9 sm:w-10 sm:h-10 rounded border border-[#e5e5e5] object-cover flex-shrink-0" alt="" />
