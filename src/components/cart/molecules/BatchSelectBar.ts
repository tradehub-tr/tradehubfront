...
export function BatchSelectBar({ totalCount, selectedCount }: BatchSelectBarProps): string {
  ...
  return `
    <div class="sc-c-batch-select-bar flex items-center justify-between gap-2 max-[380px]:gap-1.5 p-[10px_12px] sm:p-[12px_16px] rounded-md border border-[#e5e5e5] bg-white" x-data @checkbox-change="$dispatch('batch-select-toggle', { selectAll: $event.detail.checked })">
      <div class="flex items-center gap-2 max-[380px]:gap-1.5 min-w-0">
        ...
        <p class="text-sm max-[380px]:text-[13px] sm:text-base text-text-heading truncate">${t("cart.selectAll")} <span class="sc-c-batch-count text-text-tertiary ms-1">(${totalCount})</span></p>
      </div>
