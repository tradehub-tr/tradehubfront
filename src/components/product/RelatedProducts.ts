  ...
  return `
    ...
    >
      <div class="aspect-square w-full overflow-hidden bg-gray-50">
        ${
          card.imageSrc
            ? `<img class="w-full h-full object-cover transition-transform duration-300 [@media(hover:hover)and(pointer:fine)]:hover:scale-105 motion-reduce:transition-none motion-reduce:hover:scale-100" src="${escapeHtml(sanitizeUrl(card.imageSrc))}" alt="${safeName}" width="400" height="400" loading="lazy" decoding="async" />`
