        ...
        <div class="rv-modal-body overflow-y-auto px-6 pb-6 flex-1 max-sm:!px-4 max-sm:!pb-4" @click="ratingOpen = false; sortOpen = false">
          ...
          <div class="rv-filter-row flex items-center gap-1.5 sm:gap-2 flex-wrap mb-3 sm:mb-4 pt-3 sm:pt-0">
            ...
            <div class="rv-rating-dropdown relative inline-block [&.open_.rv-rating-dropdown-panel]:block" id="rv-modal-rating-dropdown" :class="{ open: ratingOpen }" @click.outside="ratingOpen = false">
              ...
              <div class="rv-rating-dropdown-panel hidden absolute top-[calc(100%+4px)] start-0 min-w-[180px] bg-[var(--color-surface,#ffffff)] border border-[var(--pd-spec-border,#e5e5e5)] rounded-lg shadow-[var(--shadow-dropdown)] z-10 overflow-hidden max-sm:!min-w-[160px]">
                <button type="button" class="rv-rating-dropdown-item flex items-center gap-2 px-4 py-2 text-[13px] text-[var(--pd-spec-value-color,#111827)] cursor-pointer transition-[background] duration-100 border-none bg-none w-full text-start hover:bg-[var(--pd-spec-header-bg,#f9fafb)] [&.active]:text-[var(--pd-tab-active-color,#cc9900)] [&.active]:font-semibold" :class="{ active: ratingFilter === 'all' }" @click="setRating('all')">${t("product.allRatings")}</button>
            ...
            <!-- Sort Dropdown -->
            <div class="rv-sort-dropdown relative inline-block ms-auto [&.open_.rv-sort-dropdown-panel]:block max-sm:!ml-0 max-sm:!w-full" id="rv-modal-sort-dropdown" :class="{ open: sortOpen }" @click.outside="sortOpen = false">
              ...
              <div class="rv-sort-dropdown-panel hidden absolute top-[calc(100%+4px)] end-0 min-w-[160px] bg-[var(--color-surface,#ffffff)] border border-[var(--pd-spec-border,#e5e5e5)] rounded-lg shadow-[var(--shadow-dropdown)] z-10 overflow-hidden max-sm:!left-0 max-sm:!right-0">
