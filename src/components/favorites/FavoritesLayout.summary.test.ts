...
vi.mock("../../utils/listingUrl", () => ({
  getListingUrl: ({ id }: { id: string }) => `/urun/${id}`,
}));
...
vi.mock("../../stores/sellerFavorites", () => ({
  getFavoriteSellers: () => [],
  removeSellerFromAll: vi.fn(),
  pruneSellerListId: vi.fn(),
}));
vi.mock("../../stores/favorites", () => ({
  getLists: () => [],
  getItems: () => items,
  getItemsByList: () => items,
  getListItemCount: () => items.length,
  getTotalCount: () => items.length,
  createList: vi.fn(),
  deleteList: vi.fn(),
  removeFromFavorites: vi.fn(),
  getListingSummary: (id: string) =>
    id === "MISSING"
      ? null
      : {
          category: "Ofis",
          supplier: { name: "Satıcı", verified: true, country: "Turkey" },
          stock_qty: 2,
          in_stock: true,
          current_price: 10,
          currency: "TRY",
