        ...
        <template x-if="banners.length > 1">
          <div>
            <button
              ...
              class="operation-slider__prev absolute top-1/2 -translate-y-1/2 start-1 z-10 w-6 h-6 flex items-center justify-center bg-white rounded-full shadow-sm border-none text-(--color-text-muted,#666666) cursor-pointer opacity-0 pointer-events-none transition-[opacity,color] duration-200 motion-reduce:transition-none [@media(hover:hover)_and_(pointer:fine)]:group-hover/opslider:opacity-100 [@media(hover:hover)_and_(pointer:fine)]:group-hover/opslider:pointer-events-auto hover:text-(--color-text-heading,#111827)"
            >
