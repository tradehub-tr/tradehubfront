...
export function UserInfoCard(props: UserInfoCardProps): string {
  ...
  return `
    <div class="p-[clamp(0.75rem,0.5rem+1vw,1.25rem)]" x-data="buyerUserInfo">
      <!-- User Profile Row + 24H Online Support Badge -->
      <div class="flex items-start justify-between gap-3 mb-4 max-sm:gap-2.5 max-sm:mb-3">
        <div class="flex items-center gap-3 max-sm:gap-2.5 min-w-0">
        ...
        <div class="inline-flex items-center gap-1 shrink-0 text-[clamp(0.625rem,0.6rem+0.1vw,0.75rem)] font-medium text-blue-600 bg-blue-50 rounded-md px-2 py-1 max-sm:px-1.5" aria-label="${t("dashboard.ariaOnlineSupport")}">
          ...
          <span class="max-sm:hidden">${t("dashboard.onlineSupport")}</span>
          <span class="hidden max-sm:inline">24H</span>
        </div>
      </div>
