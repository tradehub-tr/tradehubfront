...
function flameIcon(): string {
  return `<svg class="h-4 w-4 shrink-0 text-[var(--color-primary-500,#ff8600)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>`;
}

function renderDealSlide(deal: SideDeal, index: number): string {
  const badge =
    deal.discountPercent && deal.discountPercent > 0
      ? `<span class="absolute start-1.5 top-1.5 z-10 rounded-sm bg-[var(--color-primary-500,#ff8600)] px-1.5 py-0.5 text-[10px] font-extrabold leading-none text-white">-%${deal.discountPercent}</span>`
      : "";
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:h-[360px] xl:grid-cols-1 xl:grid-rows-[minmax(0,1fr)_auto]">
      <div id="hero-deals" class="relative flex h-[230px] min-h-0 flex-col overflow-hidden rounded-md border border-[#2a271f] bg-[#141310] xl:h-auto">
        ...
        <div class="flex items-center justify-between px-3.5 pb-2.5 pt-1.5">
          ...
          <a href="/pages/top-deals.html" class="text-[11px] font-bold text-[var(--color-primary-500,#ff8600)] no-underline appearance-none focus:outline-none hover:text-[var(--color-primary-400,#ffd75e)]">${t("heroSide.allDeals")} ›</a>
        </div>
      </div>
      <a href="/pages/dashboard/rfq-form.html"
         class="flex items-center gap-3 rounded-md border border-[var(--color-border,#e5e5e5)] bg-white p-3.5 no-underline appearance-none focus:outline-none transition-colors duration-150 hover:border-[var(--color-primary-500,#ff8600)]">
        ...
        <span class="ms-auto flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--color-primary-500,#ff8600)] text-white">
