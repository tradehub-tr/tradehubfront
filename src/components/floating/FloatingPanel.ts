...
function renderLensItem(): string {
  return `
    <button
      type="button"
      @mouseenter="if (window.matchMedia('(hover: hover) and (pointer: fine)').matches) lensOpen = true"
...
function renderLensPopup(): string {
  return `
    <div
      ...
      x-transition:enter-start="opacity-0 translate-x-4 motion-reduce:translate-x-0"
      x-transition:enter-end="opacity-100 translate-x-0"
      x-transition:leave="transition ease-out duration-150 motion-reduce:transition-none"
      x-transition:leave-start="opacity-100 translate-x-0"
      x-transition:leave-end="opacity-0 translate-x-4 motion-reduce:translate-x-0"
...
function renderChatDrawer(): string {
  return `
    ...
    <div
      x-show="chatOpen"
      x-cloak
      x-transition:enter="transition ease-[cubic-bezier(0.32,0.72,0,1)] duration-250 motion-reduce:transition-none"
      x-transition:enter-start="translate-x-full motion-reduce:translate-x-0"
      x-transition:enter-end="translate-x-0"
      x-transition:leave="transition ease-[cubic-bezier(0.32,0.72,0,1)] duration-200 motion-reduce:transition-none"
