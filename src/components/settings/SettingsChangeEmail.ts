    ...
    <div class="flex justify-center">
      <div class="w-full max-w-[640px]">
        <div class="bg-white rounded-md p-8 shadow-sm max-md:p-6 max-sm:px-4 max-sm:py-5 text-center">
          ...
          <h2 class="text-xl max-sm:text-lg font-bold mb-2 m-0" style="color:var(--color-text-primary)">${t('settings.changeEmailNav')}</h2>
          <p class="text-sm max-sm:text-[13px] mb-2 m-0" style="color:var(--color-text-secondary)">${t('settings.featureComingSoon')}</p>
          <p class="text-[13px] mb-6 m-0" style="color:var(--color-text-tertiary)">${t('settings.featureComingSoonDesc')}</p>
