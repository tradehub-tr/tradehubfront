  ...
  return `
    <div class="email-pref__item flex items-start gap-3 py-4 px-6 border-b border-(--color-border-light,#f0f0f0) last:border-b-0 max-md:px-4 max-md:py-3 max-sm:px-3">
      <label class="email-pref__checkbox relative inline-flex items-center justify-center w-5 h-5 flex-shrink-0 mt-0.5 cursor-pointer">
        ...
        <span class="email-pref__checkmark w-[18px] h-[18px] border-2 border-gray-300 rounded bg-white transition-colors duration-150 motion-reduce:transition-none flex items-center justify-center peer-checked:bg-[var(--color-primary-500,#cc6b00)] peer-checked:border-[var(--color-primary-500,#cc6b00)] peer-checked:after:content-[''] peer-checked:after:w-[5px] peer-checked:after:h-[9px] peer-checked:after:border-solid peer-checked:after:border-[var(--color-text-inverse)] peer-checked:after:border-e-2 peer-checked:after:border-b-2 peer-checked:after:rotate-45 peer-checked:after:mb-0.5"></span>
      </label>
    ...
    <div class="email-pref__category">
      <div class="flex items-center justify-between gap-6 py-5 px-6 bg-surface-muted border border-border-default border-b-0 rounded-t-lg max-md:flex-col max-md:items-start max-md:gap-3 max-md:px-4 max-sm:px-3">
        ...
        <label class="relative inline-flex w-12 h-[26px] flex-shrink-0 cursor-pointer">
          ...
          <span class="email-pref__toggle-slider absolute inset-0 rounded-[13px] bg-[var(--color-border-medium)] transition-colors duration-200 peer-checked:bg-[var(--toggle-active-bg,#cc6b00)] before:content-[''] before:absolute before:w-5 before:h-5 before:start-[3px] before:top-[3px] before:bg-[var(--color-surface,#fff)] before:rounded-full before:transition-transform before:duration-200 motion-reduce:before:transition-none before:shadow-[0_1px_3px_rgba(0,0,0,0.15)] peer-checked:before:translate-x-[22px]"></span>
        </label>
      </div>
...
function renderContent(categories: EmailCategory[]): string {
  return `
    <div class="bg-white rounded-lg p-8 max-md:p-5 max-sm:p-3.5">
      ...
      <div class="mt-5">
        <a href="#" id="email-prefs-unsubscribe-all" class="text-[13px] no-underline hover:underline" style="color:var(--color-primary-500, #cc6b00)">${t("settings.unsubscribeAll")}</a>
      </div>
    </div>
  `;
}
...
function renderError(): string {
  return `
    <div class="bg-white rounded-lg p-8 max-md:p-5 max-sm:p-3.5 flex flex-col items-center justify-center gap-3 min-h-[200px]">
      ...
      <button type="button" id="email-prefs-retry" class="text-sm bg-none border-none cursor-pointer hover:underline" style="color:var(--color-primary-500, #cc6b00)">${t("settingsUi.retry")}</button>
    </div>
  `;
}
