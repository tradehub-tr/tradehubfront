...
async function fetchProfile(): Promise<AccountData> {
  try {
    const res = await api<{ message: AccountData }>(
...
function renderEdit(d: AccountData): string {
  return `
    <div class="mb-5">
      <label class="${labelCls}" style="color:var(--color-text-secondary)">Member ID</label>
      <div class="${readonlyCls}">${d.member_id || "--"}</div>
    </div>
    <div class="grid grid-cols-2 max-sm:grid-cols-1 gap-4 mb-5">
      <div>
        <label class="${labelCls}" style="color:var(--color-text-secondary)">* ${t("settings.firstName") || "Ad"}</label>
        <input type="text" class="${inputCls}" data-field="first_name" value="${d.first_name || ""}" />
      </div>
      <div>
        <label class="${labelCls}" style="color:var(--color-text-secondary)">* ${t("settings.lastName") || "Soyad"}</label>
        <input type="text" class="${inputCls}" data-field="last_name" value="${d.last_name || ""}" />
      </div>
    </div>
    <div class="mb-5">
      <label class="${labelCls}" style="color:var(--color-text-secondary)">* ${t("settings.emailAddressField") || "E-posta"}</label>
    ...
    <div class="mb-5">
      <label class="${labelCls}" style="color:var(--color-text-secondary)">* ${t("settings.phoneLabel") || "Telefon"}</label>
    ...
    root!.innerHTML = `
      <div class="bg-white rounded-lg p-8 max-md:p-5 max-sm:px-4 max-sm:py-4">
        <div class="flex items-center justify-between mb-6">
          ...
          ${!isEditing ? `<button id="ma-edit-btn" aria-label="${t("common.edit")}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-gray-200 cursor-pointer transition-colors duration-150 hover:bg-gray-50" style="color:var(--color-text-secondary)" title="${t("settings.editBtn") || "Düzenle"}">${ICONS.edit}</button>` : ""}
        </div>
  ...
  function bindEvents() {
    ...
    document.getElementById("ma-submit")?.addEventListener("click", async () => {
      ...
      if (submitBtn) {
        ...
        submitBtn.textContent = t("settings.saving") || "Kaydediliyor...";
      }
      ...
      if (ok) {
        ...
        if (submitBtn) {
          ...
          submitBtn.textContent = t("settings.submitBtn") || "Kaydet";
        }
      }
    });
  }
