...
import { t } from '../../i18n';
import { validateTCKN, validateVKN, TR_TAX_OFFICES, getCityForTaxOffice } from '../../utils/tr-validation';
...
function renderCard(cardId: string, title: string, icon: string, viewHtml: string, editHtml: string | null, hint?: string): string {
  return `
...
function taxIdentityView(d: TaxInfoData): string {
  const typeLabel = d.taxpayer_type === 'corporate' ? t('settings.taxpayerCorporate') : t('settings.taxpayerIndividual');
  const idLabel = d.taxpayer_type === 'corporate' ? t('settings.taxIdLabelVKN') : t('settings.taxIdLabelTCKN');
  let html = `
...
function invoiceInfoView(d: TaxInfoData): string {
  const eStatus = d.e_invoice_registered ? t('settings.eInvoiceRegistered') : t('settings.eInvoiceNotRegistered');
  const invoiceType = d.invoice_type_preference === 'e_fatura' ? t('settings.invoiceTypeEFatura') : t('settings.invoiceTypeEArsiv');
  let html = '';
...
export function initSettingsTaxInfo(): void {
  ...
  function collectCardData(cardId: string): Record<string, string> {
    ...
    editEl.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>('[data-field]').forEach(el => {
  ...
  function bindEvents(): void {
    ...
    if (taxIdentityEdit) {
      taxIdentityEdit.querySelectorAll<HTMLInputElement>('input[name="taxpayer_type"]').forEach(radio => {
        radio.addEventListener('change', () => {
          const isCorp = radio.value === 'corporate';
          const tradeNameField = taxIdentityEdit.querySelector<HTMLElement>('.trade-name-field');
          const taxIdInput = taxIdentityEdit.querySelector<HTMLInputElement>('[data-field="tax_id"]');
          ...
          if (taxIdLabel) taxIdLabel.textContent = (isCorp ? t('settings.taxIdLabelVKN') : t('settings.taxIdLabelTCKN')) + ' *';
        });
      });

      // Tax office change → auto-fill city
      const taxOfficeSelect = taxIdentityEdit.querySelector<HTMLSelectElement>('[data-field="tax_office"]');
  ...
  function saveInvoiceInfo(cardId: string): void {
    ...
    current.invoice_type_preference = (data.invoice_type_preference as TaxInfoData['invoice_type_preference']) || 'e_arsiv';
