  ...
  return `
    <div class="flex items-center justify-between gap-5 p-6 max-sm:p-4 border border-border-default rounded-lg max-md:flex-col max-md:items-start max-md:gap-4">
      <div class="flex-1 min-w-0">
        <h3 class="text-[15px] max-sm:text-sm font-bold mb-2 m-0" style="color:var(--color-text-primary)">${section.title}</h3>
        <p class="text-[13px] max-sm:text-xs leading-normal m-0" style="color:var(--color-text-secondary)">${section.description}</p>
      </div>
  ...
  return `
    <div class="bg-white rounded-lg p-8 max-md:p-5 max-sm:p-3.5">
      ...
      <div class="flex border-b-2 border-border-default mb-5">
        ...
        <button class="tax-info__tab py-2.5 px-5 max-sm:px-3 text-sm max-sm:text-[13px] font-medium bg-none border-none border-b-2 border-transparent -mb-[2px] cursor-pointer transition-all" style="color:var(--color-text-secondary)" data-tab="gumruk">${t('settings.customsTab')}</button>
      </div>
      ...
      <div class="flex flex-col gap-4 mb-8" id="tax-tab-gumruk" style="display:none">
        <div class="p-10 text-center text-sm" style="color:var(--color-text-tertiary)">${t('settings.customsPlaceholder')}</div>
      </div>

      <h3 class="text-lg max-sm:text-base font-bold mb-4 m-0" style="color:var(--color-text-primary)">${t('settings.faqTitle')}</h3>
