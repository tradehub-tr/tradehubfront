  ...
  return `
    <div class="bg-white rounded-lg p-6 max-sm:p-3.5">
      <div class="flex items-center gap-2.5">
        <span class="flex items-center justify-center" style="color:var(--color-text-primary)">${card.icon}</span>
        <h2 class="text-base max-sm:text-[15px] font-bold m-0" style="color:var(--color-text-primary)">${card.title}</h2>
      </div>
...
function renderProfileHeader(): string {
  return `
    <div class="flex items-center justify-between gap-6 bg-white rounded-lg py-6 px-8 max-lg:flex-col max-lg:items-start max-lg:px-5 max-lg:py-5 max-sm:p-3.5 max-lg:gap-3">
      <div class="flex items-center gap-5 max-sm:gap-3 min-w-0 max-lg:w-full">
        <div class="relative flex-shrink-0">
          ...
          <div class="relative size-[72px] max-sm:size-12 rounded-full flex items-center justify-center border-3 border-primary-200 overflow-hidden" style="background:linear-gradient(135deg, var(--color-primary-400, #e6b212) 0%, var(--color-primary-500, #cc9900) 100%)">
            ...
            <template x-if="!userImage">
              <span class="text-[32px] max-sm:text-[21px] font-bold text-white lowercase leading-none" x-text="userInitial || '?'"></span>
            </template>
        ...
        <div class="flex flex-col gap-1 min-w-0">
          <h2 class="text-lg max-sm:text-[15px] font-bold mb-1 max-sm:mb-0 m-0" style="color:var(--color-text-primary)" x-text="userName || '...'"></h2>
          <template x-if="photoError">
            <p class="text-[11px] m-0 -mt-0.5" style="color:#dc2626" x-text="photoError"></p>
          </template>
          <div class="flex items-center gap-2 max-sm:gap-1.5 text-[13px] max-sm:text-[12px] lg:flex-wrap min-w-0">
            <span class="min-w-[110px] max-sm:hidden flex-shrink-0" style="color:var(--color-text-tertiary)">${t("settings.emailLayoutLabel")}</span>
            <span class="font-mono truncate max-lg:min-w-0" style="color:var(--color-text-primary)" x-text="userEmail || '...'"></span>
          ...
          <div class="flex items-center gap-2 max-sm:gap-1.5 text-[13px] max-sm:text-[12px] lg:flex-wrap min-w-0">
            ...
            <span class="font-mono truncate max-lg:min-w-0" style="color:var(--color-text-primary)" x-text="memberId || '...'"></span>
      ...
      <div class="flex items-center gap-4 max-sm:grid max-sm:grid-cols-2 max-sm:gap-2 flex-shrink-0 max-lg:w-full">
        <a href="#profilim" class="th-btn no-underline whitespace-nowrap max-lg:flex-1 max-lg:text-center max-sm:px-2 max-sm:py-0 max-sm:h-9 max-sm:text-[13px]">${t("settings.editProfile")}</a>
        <button @click="handleLogout()" class="inline-flex items-center justify-center px-6 max-sm:px-3 h-10 max-sm:h-9 rounded-full text-sm max-sm:text-[13px] font-semibold no-underline transition-colors duration-150 motion-reduce:transition-none whitespace-nowrap bg-none hover:underline max-lg:flex-1 max-lg:text-center cursor-pointer border-none max-sm:border max-sm:border-solid max-sm:border-[var(--color-border-default,#e5e5e5)]" style="color:var(--color-text-primary)">${t("settings.signOut")}</button>
      </div>
    </div>
  `;
}
