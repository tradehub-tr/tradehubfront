    ...
    <a :href="${dataVar} ? '/urun/' + (${dataVar}.slug || ${dataVar}.name) : '/pages/products.html'" class="block w-[calc(50%-5.5px)] group">
      <div class="w-full h-[105px] overflow-hidden rounded flex items-center justify-center" style="background-color: var(--mfr-sample-img-bg, #f5f5f5)">
        <template x-if="${dataVar} && ${dataVar}.primary_image">
          <img :src="${dataVar}.primary_image" :alt="${dataVar}.title" width="400" height="400" decoding="async" class="max-w-full max-h-full object-contain [@media(hover:hover)_and_(pointer:fine)]:group-hover:scale-105 transition-transform duration-200 motion-reduce:transition-none motion-reduce:transform-none" />
        </template>
    ...
    thumbs.length > 0
      ? thumbs
        ...
        <a href="${escapeHtml(sanitizeUrl(h.href))}" class="block w-full h-14 rounded-md overflow-hidden group bg-gray-50 relative" title="${escapeHtml(h.title || "")}">
          <img src="${escapeHtml(sanitizeUrl(h.image))}" alt="${escapeHtml(h.title || "")}" width="400" height="400" decoding="async" class="absolute inset-0 w-full h-full object-contain [@media(hover:hover)_and_(pointer:fine)]:group-hover:scale-110 transition-transform duration-200 motion-reduce:transition-none motion-reduce:transform-none">
        </a>`
          )
          .join("")
