...
export function renderPagination(
  ...
  const baseBtn =
    "inline-flex items-center justify-center w-9 h-9 text-sm rounded-md border transition-colors duration-150 select-none";
  ...
  for (let i = startPage; i <= endPage; i++) {
    ...
    pages.push(
      `<button data-page="${i}" ${isActive ? 'aria-current="page"' : ""} class="${isActive ? activeBtn : idleBtn}">${i}</button>`
    );
  }
