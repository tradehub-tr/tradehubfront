...
export function SectionCard({ children, className = "" }: SectionCardProps): string {
  return `
    <div class="bg-white dark:bg-gray-800 rounded-lg p-5 transition-shadow duration-200 [@media(hover:hover)]:hover:shadow-[0_0_12px_0_rgba(0,0,0,0.12)]${className ? ` ${className}` : ""}">
      ${children}
    </div>
  `;
}
