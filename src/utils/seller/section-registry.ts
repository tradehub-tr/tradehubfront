      ...
      <section class="storefront-section" data-section="certificates"
        x-data="{
          get verifiedCerts() {
            ...
            const raw = this.seller?.certification_list || this.seller?.certifications;
            if (!raw) return [];
            const arr = (typeof raw === 'string') ? raw.split(',') : Array.isArray(raw) ? raw : [];
            return arr.map(c => typeof c === 'string' ? ({ name: c.trim(), document: '', expiry_date: null }) : c).filter(c => c.name);
          }
        }"
      >
        <div class="max-w-[1200px] mx-auto px-4 lg:px-8 py-6">
