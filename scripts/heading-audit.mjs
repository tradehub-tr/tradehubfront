...
for (const path of STATIC) {
  ...
  let prev = 0;
  data.hs.forEach((h, i) => { if (h.level > prev + 1 && prev > 0) issues.push(`${data.hs[i-1].tag}→${h.tag} "${h.text.slice(0,30)}"`); if (h.level > prev + 1 && prev === 0) issues.push(`başta ${h.tag} "${h.text.slice(0,30)}"`); prev = h.level; });
  out.push({ path, finalUrl: page.url().replace(BASE, ""), err, count: data.hs.length, issues, hs: data.hs });
  console.log(`${issues.length ? "✗" : "✓"} ${path}${page.url().replace(BASE,"") !== path ? " → " + page.url().replace(BASE,"") : ""} [${data.hs.length}] ${issues.join(" | ")}`);
}
